...
void Debug_I2C_Status(void) {
  ...
  printf("- Instance: 0x%08lX\r\n", (uint32_t)hi2c3.Instance);
  printf("- State: %d\r\n", hi2c3.State);
...
void Test_Basic_Communication(void) {
  printf("Testing basic I2C communication...\r\n");

  // Try to read chip ID from both potential addresses
  uint8_t addresses[] = {0x28, 0x29};
  for(int i = 0; i < 2; i++) {
    uint8_t chip_id = 0;
    HAL_StatusTypeDef status = HAL_I2C_Mem_Read(&hi2c3, addresses[i] << 1,
                                                0x00, // BNO055_CHIP_ID_ADDR
                                                I2C_MEMADD_SIZE_8BIT,
                                                &chip_id, 1, 1000);

    printf("Address 0x%02X: Status=%d, ChipID=0x%02X\r\n",
